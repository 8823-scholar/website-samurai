{**
 * documents - wiki - tools_history_diff.tpl
 *}
{assign_array var='html.title.' value='変更履歴(差分)'}
...
{include file='_header.tpl' action='documents wiki tools'
    ...
    <div id='main'>
        ...
        {if $wiki2}
            <h2>{$wiki2->title|escape:'html'}@{$wiki2->revision}との差分</h2>
        {else}
            <h2>{$wiki1->title|escape:'html'}@0との差分</h2>
        {/if}
